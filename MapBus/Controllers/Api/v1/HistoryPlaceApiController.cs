...
namespace MapBus.Controllers.Api.v1
{
    public class HistoryPlaceApiController : BaseApiController
    {
        ...
        public HistoryPlaceApiController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpGet]
        [Route("/api/history_places/{token}")]
        public IActionResult Get(string token)
        {
            ...
            if (Guid.TryParse(token, out userId))
            {
                using (var conn = new NpgsqlConnection(_configuration["ConnectionString"]))
                {
                    ...
                    sql.AppendLine("select * from public.history_place ");
                    sql.AppendLine("where user_id = @UserId ");
                    sql.AppendLine("order by id desc; ");

                    var models = conn.Query<HistoryPlaceModel>(sql.ToString(), new 
                    { 
                        UserId = userId 
                    }).ToList();
        ...
        public IActionResult Create([FromBody]HistoryPlacesCreateParameter parameter)
        {
            ...
            if (Guid.TryParse(parameter.Token, out userId))
            {
                ...
                {
                    var sql = new StringBuilder();
                    sql.AppendLine("INSERT INTO public.history_place ");
